     				...
     				<table class="table table-hover" style="text-align: center">
                        ...
                        <?php 
	                        ...
	                        ?>
	                            ...
	                            '<?php echo $gender?>',<?php echo $age?>,'<?php echo $text?>',<?php echo $read?>)"><span class="glyphicon glyphicon-pencil"></span></div></td>
	                        	</tr>
	                        <?php
	                        	}
	                        ?>
...
<script>
	var currentFromId;
	var mid;
	function showdetail(id,from,name,gender,age,text,read){
		currentFromId=from;
		mid=id;
		$("#myModal").modal();
		$("#newstart").val(text);
    ...
    $("#submit").click(function(){
		...
		xmlHttp.setRequestHeader("Content-Type",
				"application/x-www-form-urlencoded");
		xmlHttp.send("fromId="+did+"&toId="+currentFromId+"&mid="+mid+"&reply="+reply);
    });
    var xmlHttp=null;
		...
		if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete") {
			...
			if (xmlHttp.status == 200) {
				...
				if(data=="update success"){
							return;
				}else if(data=="send success"){
					alert("回复发送成功");
				}else{
					alert("发生了点小错误，请重试");
				}
				window.location.href="advice_write.php";
			}
		}
	}
