...
function get_all_doctors(){
	...
	var doctor_container=document.getElementById('doctor-container');

	var url="/api/v1/doctors";
	...
	$.getJSON(url,function (data) {   
		...
		for(var i=0;i<length;i++){
			var id = doctors[i]['id'];
			var nickname = doctors[i]['nickname'];
			var gender = doctors[i]['gender'];
			var job = doctors[i]['job'];
			var profession = doctors[i]['profession'];
			...
			if(window.addEventListener){ // Mozilla, Netscape, Firefox
        		link.addEventListener('click',function(){get_doctor(id)}, false);
    		} else { // IE
        		link.attachEvent('onclick',  function(){get_doctor(id)});
    		}
			...
			doctor_container.appendChild(link);
		}
	});
...
function get_doctor(id){
	...
	//首先删除doctor-container的所有子节点
	var doctor_container=document.getElementById('doctor-container');

	while(doctor_container.hasChildNodes()) //当div下还存在子节点时 循环继续
    {
        doctor_container.removeChild(doctor_container.firstChild);
    }

	var url="/api/v1/doctors/"+id;

	$.getJSON(url,function (data) {   

		var doctor=data['doctor'][0];

		var id=doctor['id'];
		var nickname=doctor['nickname'];
		var gender=doctor['gender'];
		var address=doctor['address']
		var job=doctor['job']
		var profession=doctor['profession']
		var birthday=doctor['birthday']
...
*/
		...
		doctor_container.appendChild(left);
		doctor_container.appendChild(right);
